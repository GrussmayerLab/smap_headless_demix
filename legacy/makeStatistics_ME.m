...
% frames
for k=datrange
    slegend=[modetxt num2str(datrange(k))];
    frames=locs.frame;
    if isempty(frames)
        continue
    end
...
% if p.checkphot
%     for k=datrange
%         phot(phot<p.photrange(1))=[];
%         if length(p.photrange)>1
%              phot(phot>p.photrange(2))=[];
%         end
%     end
%     pr=p.photrange;
% else
...
for k=datrange
    sphot{end+1}='';
    sphot{end+1}=[num2str(k) '.' modetxt ];
    Nloc(k)=length(phot);
    meanphot(k)=mean(phot);
%     N1(k)=sum(phot>phot1);
%     N2(k)=sum(phot>phot2);
    inrange=phot>p.photrange(1)&phot<p.photrange(2);
    meanphotrange(k)=mean(phot(inrange));
    medianphotrange(k)=median(phot(inrange));
...
%     dat(k)=fitexpphot(hphot,[],ploton);
%     dat(k)=meanexphere(phot(inrange),hphot,p.photrange,ax1,mmax);
    dat(k)=meanexphere(phot,hphot,p.photrange,ax1,mmax);
...
for k=datrange
    ...
    [~,indrise1]=find(hlocp.h>0.2,1,'first');
    indrise1=min(4*indrise1,length(hlocp.h));
    imaxx=max(hlocp.h(1:indrise1));
    indrise=find(hlocp.h>imaxx/2,1,'first');
    risingedge=hlocp.n(indrise);
    ...
    rn=indrise:min(ind+dwin,length(hlocp.h));
    fpol=fit(hlocp.n(rn)',hlocp.h(rn)','poly3');
    smxf=fzero(@(x) 3*fpol.p1*x.^2+2*fpol.p2*x+fpol.p3,smx);
    if ~isempty(ax2)
        nf=hlocp.n(rn(1)):0.05:hlocp.n(rn(end));
        plot(nf,fpol(nf),'k:')
    end
    
    stat.locprec.max(k)=smxf;
    slp{end+1}=['max: ' num2str(smxf,3)];
    slp{end+1}=['median: ' num2str(median(locp),3)];
    stat.locprec.median(k)=median(locp);
    %risng edge
    geom=geomean(loch);
...
slt={'on-time'};
for k=datrange
    inrange=lifetime>p.lifetimerange(1)&lifetime<p.lifetimerange(2);
    slt{end+1}='';
    slt{end+1}=[num2str(k) '.' modetxt ];
%     dat(k)=fitexpphot(hlifet,2,ploton);
    dat(k)=meanexphere(lifetime,hlifet,p.lifetimerange,ax3,mmax);
    slt{end+1}=(['texp'  ' = ' num2str(dat(k).mu,3)]);
    slt{end+1}=(['meanrange'  ' = ' num2str(mean(lifetime(inrange)),3)]);
    slt{end+1}=(['meanall'  ' = ' num2str(mean(lifetime),3)]);
...
for k=1:length(v)
    ...
    qq=myquantilefast(v,[1-quantile,quantile],30/(1-quantile));
    else
    qq=quantile;
    end
    q(k)=qq(2);q0(k)=qq(1);
...
%         sum(v)
        ...
        his.h=h(2:end-1)/mmax;
        his.n=nphot(2:end-1);
        if ~isempty(ax)
        plot(nphot(2:end-1),h(2:end-1)/mmax)
        hold on
        end
        slegend{end+1}=[modetxt];
        mmo=mmax;
    end
end
if ~isempty(ax)
